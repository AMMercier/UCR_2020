...
from GraphVis import draw_network, draw_ran_geo
...
def ConfigGraphGen(k):
    if sum(k) % 2 != 0: #The sum of the degrees must be even
        raise ("Please use a degree distribution that sums two an even number.")
    else:
       L = []
...
#Input:
# s - list of community sizes
# W - matrix of probabilites where W_ij is the prob of connecting community i and j
#Output:
# A - adj matrix of Configuration network
def SBM(s,W):
                    ...
                    for y in range(len(C2)):
                        if x <= y:
                            ...
                            if theta < pr:
                                ...
                                m = C2[y]
                                if n != m:
                                    A[n][m] += stt.uniform.rvs()
                                else:
                                    A[n][n] += 2*stt.uniform.rvs()
    C = np.transpose(A)
    C = np.triu(C, 1)
    return A+C

#Measure euclidian distance between two vectors
#Input:
# x - vector 1
# y - vector 2
#Output:
...
def Lattice(n,shape):
    if shape[0]*shape[1] != n:
        raise("Your dim must equal the number of nodes.")
    pos = {}
    N=list(range(n))
    for m in range(n):
        tick = 0
